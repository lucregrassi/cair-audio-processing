...
global data 
global cb_data_received 
cb_data_received = False
data = []

def callback(input_data, frame_count, timeinfo, flags):
    global data
    global cb_data_received
    data.append(input_data)
    cb_data_received = True
...
class Recorder:
    ...
    def open_stream(self):
        ...
        if input_device == -1:
            ...
            self.stream = self.p.open(format=audio_format, channels=channels, rate=rate, input=True, output=False, stream_callback=callback,
                                      frames_per_buffer=frames_per_buffer, start=False)
            print("stream open°°°°°°°°°°°°°°°°°°", chosen_device)
        else:
    ...
    def record(self):
        ...
        prev_stream_ready = time.time() 
        while current <= end:
            count = 0
            #while self.stream.get_read_available() < chunk:
            #    #print(f"RECORD while: stream data available: {self.stream.get_read_available()}")
            #    time.sleep(0.1)
            #    count = count + 1
            #    if count == 10:
            #        print("RECORD!!!!!!! closing stream !!!!!!!!!!!!!!!!!!!!!!!!!!")
            #        self.stream.close()
            #        self.p.terminate()
            #        print("RECORD******* reopening stream **********************")
            #        self.open_stream()
            #        count = 0
            #        print("RECORD ******* STREAM REOPPENED
            while (cb_data_received == False):
                print("RECPRD *** WAITING for data")
                time.sleep(0.01)
            cb_data_received = False    
            ...
            #print(f"RECORD: stream data available: {self.stream.get_read_available()}")
            #print(f"RECORD: stream time diff: {stream_ready_time-prev_stream_ready}")
            prev_stream_ready = stream_ready_time
        ...
        while True:
            ...
            while True:
                ...
                while current <= end:
                    count = 0
                    #while self.stream.get_read_available() < chunk:
                    #    #print(f"LISTEN while: stream data available: {self.stream.get_read_available()}")
                    #    time.sleep(0.1)
                    #    count = count + 1
                    #    if count == 200:
                    #        print("LISTEN !!!!!!! closing stream !!!!!!!!!!!!!!!!!!!!!!!!!!")
                    #        self.stream.close()
                    #        self.p.terminate()
                    #        print("LISTEN ******* reopening stream **********************")
                    #        self.open_stream()
                    #        count = 0
                    #        print("LISTEN ******* STREAM REOPPENED **********************")
                    
                    while (cb_data_received == False):
                        print("LISTEN *** WAITING for data")
                        time.sleep(0.01)
                    cb_data_received = False   
                    audio_input = data
                    data = []
                    #print(f"LISTEN: stream data available: {self.stream.get_read_available()}")
                    #print(f"LISTEN: stream data available before: {self.stream.get_read_available()}")
                    #audio_input = self.stream.read(chunk, exception_on_overflow=True)
                    stream_ready_time = time.time() 
                    #print(f"LISTEN: stream data available after : {self.stream.get_read_available()}")
                    #print(f"LISTEN: stream time diff: {stream_ready_time-prev_stream_ready}")
                    prev_stream_ready = stream_ready_time
                    ...
                    if rms_val > rms_threshold:
                        ...
                        if timeout_end_time - timeout_start_time > TIMEOUT:
                            ...
                            #self.stream.close()
                            #print_colored("STREAM CLOSED", "red")
